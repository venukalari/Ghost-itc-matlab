function AB_SN = SNsolve2_specbin(lambda,t_low,t_high,SN,ZD,resoln,seeing,SB,N_mirror,SR_sky,bin_spat,N_sub,plotQ)
%
%   Calculate (and optional plot) AB for given S/N at range of exp times, at a single wavelength
%   * This version does 2x spectral binning (so SR only) *
%
%   Input parameters:
%   -----------------
...
%
    ...
    assert(strcmp(resoln,'SR'),'Spectral binning only valid for SR!')
...
% 
    ...
    (SNmain_specbin(lambda,AB,t_exp,ZD,resoln,seeing,SB,N_mirror,SR_sky,bin_spat,N_sub,plotQ2) - SN);
        plotQ2 = 0;
...
%             
        [SN_calc(i), vars(:,i)]=SNmain_specbin(lambda,AB,t_exp,ZD,resoln,seeing,SB,N_mirror,SR_sky,bin_spat,N_sub,0);
        AB_SN(i) = AB;
...
%
    if plotQ && n_time ~= 1
        ...
        hold on
        plot(linspace(t_low,t_high,n_time),vars(2,:),'blue')
        plot(linspace(t_low,t_high,n_time),vars(3,:),'black')
        plot(linspace(t_low,t_high,n_time),vars(4,:),'green')
        xlabel(['Exposure time at ',num2str(lambda),' nm  /s'])
        ylabel('Variances')
        axis_vec = axis;
        plot([3600 3600],[axis_vec(3) axis_vec(4)],'color',[0.75 0.75 0.75],'LineWidth',1.5)
