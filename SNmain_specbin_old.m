function [lambda_2,SN,vars]=SNmain_specbin(lambda,AB,t_exp,ZD,resoln,seeing,N_mirror,SR_sky,bin_spat,N_sub,plotQ)
%
...
%   Output parameters:
%   ------------------
%   lambda_2    : output wavelength grid - at double the spacing of input lambda
...
%    
    ...
    lambda_2 = (lambda(1:2:end-1) + lambda(2:2:end))/2; % Binned 2x
    [dim1,~] = size(lambda);
    ...
    assert(resoln == 'SR','Spectral binning is calculated for standard resolution only!')
    [~,n_lambda_2] = size(lambda_2);
    vars = zeros(4,n_lambda_2);
%    
    area_IFU = area_SR;
    slit_len = Npix_SR;
...
%   value, which is OK)
%   
    log_f_lambda = -AB/2.5 - 2*log10(lambda_2*10) - 2.408/2.5;  % get f_lambda
    f_lambda = 10.^log_f_lambda;  % in erg /s /cm^2 /Å
%
    rate0 = f_lambda.*lambda_2*1E-9/(6.6256E-27*2.99792E8);
               % photon rate incident on top of atmosphere, phot /cm^2 /Å /s
    rate1 = rate0*area_tel;  %     phot /Å /s  for whole telescope area
%    
    extinc_frac = Extinc_Paranal(lambda_2,ZD,0);  % Paranal extinction; fraction passed
    rate2 = rate1.*extinc_frac; % after atmos extinction; phot /Å /s
%    
    rate3 = rate2.*GS_reflectivity(lambda_2,N_mirror,0); % after telescope mirrors; phot /Å /s  
    rate4 = rate3*IFU_trans(resoln,seeing); % drop by IFU injection loss
    rate5 = rate4.*GHOST_cable_eta(lambda_2); % drop by Cass Unit and cable throughputs 
    rate6 = rate5.*sgr_throughput(lambda_2,0); % drop by sgr and detector throughputs    
    rate7 = rate6.*BlazeFunction(lambda_2);    % drop by individual order blaze functions
%    
    RD = 2*nmperpix(lambda_2)*10;  % Reciprocal dispersion, Å/pix for binned pixels
...
%
    sky_flux = Sky_contin(lambda_2,0);  % sky *continuum* flux, erg /s /Å /cm^2 /arcs^2
    sky_rate1 = sky_flux.*lambda_2*1E-9*area_tel*t_exp/(6.6256E-27*2.99792E8); % phot /Å /arcs^2
    sky_rate2 = sky_rate1.*GS_reflectivity(lambda_2,N_mirror,0);   % losses through the system
    sky_rate3 = sky_rate2.*GHOST_cable_eta(lambda_2).*sgr_throughput(lambda_2,0);
    sky_rate4 = sky_rate3.*BlazeFunction(lambda_2);  
...
%    
    dark0 = (lambda_2 < lambda_cross)*dark_blue + (lambda_2 >= lambda_cross)*dark_red; %  e- /pix /hr  
    dark1 = dark0*t_exp/3600; %  e- /pix 
    dark2 = 2*dark1*slit_len;   %  e- /pix, summed over slit length
%
    ron_rms = (lambda_2 < lambda_cross)*RON_blue + (lambda_2 >= lambda_cross)*RON_red;
...
%
    ...
    B2_1 = B2lookup(lambda_2,resoln,1);     % sum(B^2) on the original unbinned wavelength grid
    B2_2a = (B2_1(1:2:end-1) + B2_1(2:2:end))/2; % Binned 2x
    B2 = 0.5*(1 + B2_2a); % i.e. on av sum(B^2) will have half the excess over 1.0 cf unbinned  
%
